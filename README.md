...
```js
import '@miniature-broccoli/ds/tokens.css';
import '@miniature-broccoli/ds/button';
import '@miniature-broccoli/ds/input';
```
...
Typography: CSS classes `.ds-title`, `.ds-body`, `.ds-body-sm` (no `ds-text` element).
...
- **`ds-select`**: pass `options={[{ value, label }]}` — do not slot `<option>`.
- **`ds-modal`**: native `<dialog>`.
...
`@miniature-broccoli/ds` is `private` for now. Peer dependency: `lit@^3.2.0`.
